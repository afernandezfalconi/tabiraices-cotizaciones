...
function addCorsHeaders(response: Response, origen: string | null): Response {
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      ...
      if (pathname.startsWith('/landing/') || pathname.startsWith('/api/cotizaciones')) {
        ...
        response = await handleHoldsRequest(
          request,
          env.INVENTORY_KV,
          currentSettings,
          env.TABIRA_USUARIOS
      ...
      if (error instanceof AuthError) {
        ...
        return addCorsHeaders(
          json({ success: false, error: error.message, codigo: error.codigo }, status),
          origen
        );
      }
