...
export async function handleUsuariosRequest(
  ...
  if (ruta === '/api/actividad' && metodo === 'GET') {
    ...
    const items = desde
      ? registros.filter((r: any) => r && r.usuario === desde)
      : registros;
    ...
    for (const r of registros as any[]) {
      ...
      u.total++;
      if (!u.ultimo || r.fecha > u.ultimo) u.ultimo = r.fecha;
      if (r.accion === 'login') u.accesos++;
      else if (r.accion === 'cotizacion_crear') u.cotizaciones++;
      else if (r.accion === 'cotizacion_compartir') u.compartidas++;
      else if (r.accion === 'landing_vista') u.vistasDelCliente++;
    ...
    return ok({
      items,
      resumen: Object.values(porUsuario).sort((a: any, b: any) =>
