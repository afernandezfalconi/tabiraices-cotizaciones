 ...
 */
function landingHTML(c: Cotizacion): string {
...
export async function handleQuotesRequest(
  ...
  if (metodo === 'GET' && mLanding) {
    ...
    if (!c) {
      return new Response(
        '<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8"><title>No encontrada</title></head><body style="font-family:system-ui;text-align:center;padding:60px;color:#555"><h1 style="font-size:20px">Cotización no encontrada</h1><p>El enlace expiró o ya no está disponible.</p></body></html>',
  ...
  if (mId && ['GET', 'DELETE'].includes(metodo)) {
    ...
    if (!c) return err('Cotización no encontrada', 404, 'NO_ENCONTRADA');
