...
export class UsersService {
    ...
    async login(usuarioRaw, password) {
        ...
        if (!u.activo)
            return { ok: false, codigo: 'INACTIVO' };
        ...
        await this.kv.put(`token:${token}`, JSON.stringify({ userId: u.id, gen: u.gen || 0, creado: new Date().toISOString() }), { expirationTtl: TTL_TOKEN });
     ...
     */
    async minutosBloqueado(usuario) {
        const r = await this.kv.get(`intentos:${usuario}`, 'json');
        if (!r || r.n < MAX_INTENTOS)
            return 0;
        const restan = new Date(r.hasta).getTime() - Date.now();
        return restan > 0 ? Math.ceil(restan / 60000) : 0;
    }
    async registrarFallo(usuario) {
        try {
            const r = (await this.kv.get(`intentos:${usuario}`, 'json')) || { n: 0, hasta: '' };
            const n = r.n + 1;
    ...
    async porId(id) {
        return this.kv.get(`user:${id}`, 'json');
    }
    async existeAlguno() {
    ...
    async crear(datos) {
        ...
        if (await this.kv.get(`idx:usuario:${usuario}`))
    ...
    async actualizar(id, cambios) {
        ...
        if (cambios.nombre)
            u.nombre = limpiar(cambios.nombre, 120);
        if (esRolValido(cambios.rol))
            u.rol = cambios.rol;
    ...
    async cambiarPasswordPropia(id, actual, nueva, tokenActual) {
        const u = await this.porId(id);
        if (!u)
        ...
        await this.kv.put(`token:${tokenActual}`, JSON.stringify({ userId: id, gen: u.gen, creado: new Date().toISOString() }), { expirationTtl: TTL_TOKEN });
        await this.cerrarSesiones(id, tokenActual);
    }
    /** Cierra las sesiones del usuario. `conservar` deja viva esa sola. */
    async cerrarSesiones(userId, conservar) {
        const lista = await this.kv.list({ prefix: 'token:', limit: 1000 });
        for (const k of lista.keys) {
            if (conservar && k.name === `token:${conservar}`)
                continue;
            const s = await this.kv.get(k.name, 'json');
